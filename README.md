# METAS CORPORATION 사업문의 페이지

케이브릿지 해외특송 페이지(example.zip)와 동일한 구조로 만든
메타스코퍼레이션 고객 유치용 웹페이지입니다.

## 포함 파일
- `index.html` : GitHub Pages용 메인 페이지 (사업문의 폼 + 사업분야 + 블로그)
...
※ URL을 교체하기 전에는 `사업문의 접수하기` 버튼이
   business@metas-corp.com 앞으로 메일 작성 창을 여는 방식으로 동작합니다.
   URL 교체 후에는 Apps Script로 전송되고 `문의가 접수되었습니다` 팝업이 표시됩니다.
...
2. `blog/posts.json` 맨 앞에 글 정보 추가
   ...
   - 썸네일을 쓰려면 1200 x 630 px 이미지를 `blog/images/`에 넣고 경로를 적습니다.
